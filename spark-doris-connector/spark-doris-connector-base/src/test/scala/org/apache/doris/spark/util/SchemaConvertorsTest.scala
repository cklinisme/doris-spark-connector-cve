...
class SchemaConvertorsTest {
  ...
  def toCatalystTypeTest(): Unit = {
    ...
    // Upstream reads LARGEINT as text; a signed 128-bit value can exceed Spark's 38-digit decimal.
    Assert.assertEquals(DataTypes.StringType, SchemaConvertors.toCatalystType("LARGEINT", -1, -1))
